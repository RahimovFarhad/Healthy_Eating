      ...
      <div style="background:#fff;border:1px solid #d4e7d4;border-radius:12px;padding:2.5rem;box-shadow:0 2px 12px rgba(27,77,27,0.06);width:100%;">
        ...
        <form @submit.prevent="handleVerify">
          ...
          <button type="submit" class="btn btn-gf w-100 mb-3" :disabled="loading || code.length !== 6">
            {{ loading ? 'Verifying...' : 'Verify & Create Professional Account' }}
          </button>
        ...
        <div class="mt-4 pt-3 text-center" style="border-top:1px solid #e8f4e6;font-size:0.85rem;color:#6a8f6a;">
          Wrong email? 
          <RouterLink to="/register/professional" style="color:#2e7d32;font-weight:600;text-decoration:none;">Start over</RouterLink>
        </div>
      </div>
    </div>
...
import { login, apiFetch, tryRefresh, authError } from '../auth.js'
...
async function handleVerify() {
  ...
  try {
    ...
    if (!verifyRes.ok) {
      error.value = verifyData.message || 'Verification failed'
      return
    }
    ...
    success.value = 'Professional account created successfully!'
